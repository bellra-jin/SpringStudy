...
public class ManyToOneAssociationTests {
    ...
    // Association Mappiong은 Entity간의 관계를 매핑하는 것을 의미한다.
    // 이를 통해 객체를 이ㅛㅇ해 데이터베이스 테이블 간의 관계를 매핑할 수 있다.

    // 다중성에 의한 분류
    // 인관관계가 있는 객체 관계에서 실제로 연관을 가지는 객체의 수에 따라 분류
    // N:1(ManyToOne), 1:N(OneToMany), 1:1(OneToOne), N:N(ManyToMany)

    // 방향에 따른 분류
    // 테이블의 연관 관계는 외래키를 이요하여 양방향 연관관계의 특징을 가진다.
    ...
    @CsvSource({"1, 커피", "2,기타"})
    @Transactional
    // 지연 로딩의 경우 트렌젝션이 종료되면 session이  close 되면서 준영속 상태가 된다.
    // 엔티티 매니저가 관리하지 않는 객체가 되는 것이기 때문에 세션이 종료되었다는 에러가 발생한다.
    // 지연 로딩을 이용하는 경우 동일한 트랜젝션 내에서 사용해야한다.
