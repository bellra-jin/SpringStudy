...
public class JoinTests {
     ...
     * 2. 패치 조인 : JPQL에서 성능 최적화를 위해 제공하는 기능으로 연관ㄱ된 엔티티나 컬렉션을 한 번에 죄회할 수 있다.
    ...
    void testFindAllMenusUsingInnerJoin() {
        ...
        // 지연 로딩 때문에 발생하게 됨
        // fetchType을 EAGER로 설정해도 동일한 문제가 발생한다. (fetch join으로 해결함, 혹은 dto 프로젝션 활용)
    ...
    void testFindAllMenusUsingFetchJoin() {
        ...
        // 하지만 패치 조인을 사용하는 경우 paging을 하면 안된다.
    ...
    void testFindAllMenusInCategory() {
        ...
        // 카테고리를 조회하고 그 카테고리 메뉴 목록을 조회한 후
        // 해당 메뉴들이 카테고리를 기쥔으로 다시 카테고리르 조회하기 때문에 중복된 데이터가 조회된다.
